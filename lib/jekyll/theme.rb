    ...
    class Theme
        ...
        def root
            ...
            if(!gemspec.nil?)
                ...
                @root ||= File.realpath(DEFAULT_THEME_DIR + name)
            end

        rescue Errno::ENOENT, Errno::EACCES, Errno::ELOOP
            raise Jekyll::Errors::InvalidPathError,
                "#{self.class}: Theme path does not exist, is not accessible or includes a symbolic link loop"
        end
        ...
        def runtime_dependencies
            ...
            if !gemspec.nil?
                gemspec.runtime_dependencies.each { |dep| runtime_dep << dep.name }
            else
                runtime_dep = Jekyll::Theme::DEFAULT_THEME_PLUGINS
            end
        ...
        def log_realpath_exception(err, folder)
            ...
            case err
            when Errno::EACCES
                Jekyll.logger.error "#{self.class}:", "Directory '#{folder}' is not accessible."
            when Errno::ELOOP
                Jekyll.logger.error "#{self.class}:", "Directory '#{folder}' includes a symbolic link loop."
            end
        end
